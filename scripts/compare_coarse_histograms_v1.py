...
def measure(q):
    ...
    result = {
        ...
        "median_equivalent_gamma": gamma, "median_equivalent_lambda": A * gamma,
...
def overview(current, output):
    ...
    axes[0].legend(loc="upper right", fontsize=9, framealpha=.92)
    for key, label, marker, color, offset in (
    ...
    axes[1].legend(loc="upper right", fontsize=9, framealpha=.92)
...
def channel_page(channel, metadata, arrays, eras, monthly, output, atlas):
    ...
    fig.subplots_adjust(left=.075, right=.975, top=.815, bottom=.11, hspace=.39, wspace=.24)
    for ax in axes.flat:
        style_axis(ax)
    ...
    axes[0,0].legend(fontsize=7.8, loc="upper right")
    ...
    if supported:
        x = [month_date(r["month_index"]) for r in supported]
        med = np.array([r["median"] for r in supported])
        qlo = np.array([r["quantiles"][str(PROBS[3])] for r in supported])
        qhi = np.array([r["quantiles"][str(PROBS[7])] for r in supported])
        ...
        axes[1,0].xaxis.set_major_locator(mdates.AutoDateLocator(minticks=3, maxticks=5))
    ...
    axes[1,0].set_title(f"Current-era monthly structure ({len(supported)}/{len(months)} supported)",loc="left",fontsize=11,weight="bold")
    ...
    axes[1,1].set_title("All saved eras",loc="left",fontsize=11,weight="bold")
    ...
    fig.text(.075,.055,"Model curves are integrated over the displayed bins. Zoom normalization retains all frames. Monthly shading is data spread, not uncertainty.",fontsize=8.5,color="#465e6b")
    fig.text(.075,.023,"Retrospective median matching; ideal independent Gaussian projections and clean references. Era labels do not independently establish transmitter state.",fontsize=8.5,color="#465e6b")
    for ext in ("png","pdf"):
        fig.savefig(output.with_suffix("."+ext),facecolor="white")
...
def main():
    ...
    report={"schema":"canfar-coarse-histogram-comparison-v1","created_utc":datetime.now(timezone.utc).isoformat(),
            ...
            "totals":{"channels":len(current),"eras":len(all_eras),"monthly_groups":len(all_months),"acquisition_era_groups":len(all_acquisitions),"current_frames":sum(r["n_frames"] for r in current),"all_era_frames":sum(r["n_frames"] for r in all_eras),"histogram_eligible_all":sum(r["counts"]["histogram_eligible"] for r in channel_records)},
