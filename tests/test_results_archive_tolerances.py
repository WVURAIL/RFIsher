...
def test_constants_come_from_the_one_home_and_the_ledger_only_checks_alpha_par(tmp_path):
    ...
    assert math.isnan(rows[14].r_tol_fs8) and rows[14].fs8_status == tol.FS8_UNPRICED
    ...
    assert by[14]["r_tol_fs8"] == "" and by[35]["bins"] == "5" and float(by[35]["r_tol_dilation"]) == published.TOL_APERP[35]
...
def test_without_a_ledger_the_constants_still_price_every_channel(tmp_path):
    ...
    assert sum(math.isfinite(r.r_tol_fs8) for r in rows) == 10
