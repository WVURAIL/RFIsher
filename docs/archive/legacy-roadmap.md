...
detector work, data-collection plans, and RFIsher forecast tasks. Git history is
the authoritative archive for that detailed chronology. This short record
preserves only the unresolved context that can still affect RFIsher results.
...
- Correlation-time measurements still need contiguous scans, first for
  ...
  survey-product producer; RFIsher consumes their resulting provenance and
  residual budgets.
...
- New measurement products must record their detector rule, kernel identity,
  frequency target, and survey epoch. RFIsher deliberately refuses to infer those
...
- `fine_operating_point.py`, `plot_convergence.py`, and `three_worlds.py` are
  ...
  helpers into `rfisher` if a workflow is promoted to a maintained interface.
